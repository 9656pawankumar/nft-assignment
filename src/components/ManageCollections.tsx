...
import { Card, CardContent, CardActions, Button, Grid, Typography, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
export default function BrowseNFT() {
  ...
  <span className="goBackLink"  onClick={handleNavigateView}>{'< Go Back'}</span>
  <span className="headingText">Manage Collections</span>
</div>
       
<div style={{ padding:'20px',display: 'flex', justifyContent: 'center',height:'100%',backgroundColor:'rgba(255, 255, 255, 0.3)'}}>
<Grid container spacing={5} justifyContent="center">
  {nftCollections?.map((nc: any, id: number) => (
    <Grid item xs={12} md={6} key={id} sx={{ margin: '0 auto', textAlign: 'center' }}>
      <Card style={{maxWidth:'40rem'}}>
        <CardContent>
          <Typography variant="h5" component="h2">
            <b>Collection Name: </b>{nc?.collectionName}
          </Typography>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', overflowX: 'auto' }}>
        ...
        <CardActions style={{ justifyContent: 'center' }} >
          <Button id={`addBtn-${id}`}   size="small" style={{  marginBottom: '10px',
                ...addCollectionsStyle,
                fontWeight: 'bold',
                transition: 'background-color 0.3s, color 0.3s'}}
              onClick={() => {
            const dialog = document.getElementById(`addMoreDialog-${id}`);
  ...
  style={{ backgroundColor: 'white', width: '300px', position: 'relative' }}
>
  <button
    onClick={() => {
      const dialog = document.getElementById(`addMoreDialog-${id}`);
