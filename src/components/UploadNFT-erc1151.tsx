...
export default function UploadNFT2({
  ...
  async function mintNFTs(tokenURIs: string[], collectionId: number) {
    ...
    try {
      ...
      for (const tokenURI of tokenURIs) {
        ...
        handleClick(`Your NFT is minted!`, 'success');
      }
    } catch (error) {
  ...
  async function getIPFSUrl(file: any) {
    if (file) {
      ...
      handleClick(`No files are uploaded!`, 'error');
      return;
    }
  }
  ...
  async function createCollection() {
    try {
      ...
      await transaction.wait();
      handleClick(`Your Collection is created! Wait for the NFT to be minted!`, 'success');
    } catch (error) {
  ...
  const handleUpload = async (e: any) => {
    ...
    await mintNFTs(tokenURIs, collectionLength);
  };
...
const goBack = () => {
  ...
  return (
    <div style={{ width: '50%', height: '50%', position: 'absolute', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', padding: '20px', borderRadius: '10px', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.1)' }}>
   <button onClick={goBack} style={{ color:'black', fontWeight:'bolder',position: 'absolute', top: '10px', left: '10px' }}>&lt;Go Back</button>
      <h1 style={{ fontSize:'25px',fontWeight:'bolder',marginBottom: '20px', color: '#333' }}>Add a collection - ERC 1151</h1>
  ...
  <div className="flex justify-center">
    <label htmlFor="uploadInput" style={{ color: '#000', fontWeight: 'bold' }}>Upload Image</label>
  </div>
  <div className="flex justify-center">
